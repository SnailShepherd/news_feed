...
SOURCE = "Интерфакс-Недвижимость"
BASE = "https://www.interfax-russia.ru/realty/news"

def harvest(fetch: Fetcher, limit: int = 30):
    resp, err = fetch.get(BASE)
    if err:
        log(f"ERROR !! Failed: Интерфакс-Недвижимость ({err})")
        return []
    soup = soup_html(resp)
    items = []
    for a in soup.select("a[href*='/realty/news/']"):
        href = a.get("href")
