...
def harvest(fetch: Fetcher, limit: int = 40):
    ...
    if err:
        log(f"ERROR !! Failed: Российская газета: Экономика ({err})")
        return []
    soup = soup_html(resp)
    items = []
    for a in soup.select("a[href*='/202']"):  # как правило, материалы имеют в url год
        href = a.get("href")
        ...
        if not err2:
            s2 = soup_html(resp2)
            t = s2.select_one("time[datetime], time, .ArticleHeader_date__*")
            if t:
