...
SOURCE = "РИА Недвижимость: лента"
BASE = "https://realty.ria.ru/lenta/"

def harvest(fetch: Fetcher, limit: int = 30):
    resp, err = fetch.get(BASE)
    if err:
        log(f"ERROR !! Failed: РИА Недвижимость: лента ({err})")
        return []
    soup = soup_html(resp)
    items = []
    for a in soup.select("a[href*='/realty/']"):
        ...
        if not err2:
            s2 = soup_html(resp2)
            t = s2.select_one("time[datetime], time, .article__info-date")
            if t:
